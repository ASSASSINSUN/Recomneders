...
# Activation Unit，DIN 的activation unit算权重方式：把[q, v, q-v, q*v]过MLP
#输入值 0 是否是应被屏蔽的特殊“填充”值。这在使用可能采用可变长度输入的循环层时很有用。如果是True这样，则模型中的所有后续层都需要支持屏蔽，否则将引发异常。如果将 mask_zero 设置为 True，则索引 0 不能在词汇表中使用（input_dim 应该等于词汇表的大小 + 1）。
...
# 去掉维度为1的，即去掉0，防止做外积的时候全部变成0
candidate_emb_layer = tf.squeeze(candidate_emb_layer,axis=1)
#因为user_behaviors_emb_layer的输出是5个embedding，所以这里重复输入5次，把维度对其才能进行外积操作
